...
async function withMessage (promise, message) {
  // Find out if current version of susybraid-core supports revert reason i.e >= 2.2.0.
  // https://github.com/susy-knotsuite/susybraid-core/releases/tag/v2.2.0
  const nodeInfo = await susyweb.sof.getNodeInfo();
  const matches = /TestRPC\/v([0-9.]+)\/sophon-js/.exec(nodeInfo);
  if (1 in matches && semver.satisfies(matches[1], '>=2.2.0')) {
    ...
    console.log(`${colors.white.bgBlack('susyufo-test-helpers')} ${colors.black.bgYellow('WARN')} \
shouldFail.reverting.withMessage: current version of Susybraid (${matches[1]}) doesn't return revert reason.`);
